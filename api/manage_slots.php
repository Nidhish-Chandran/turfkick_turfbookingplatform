...
if ($method === 'GET') {
    ...
    if ($action === 'save_day') {
        ...
        if (!in_array($day, $allowed_days, true)) {
            send_json_response('error', 'Invalid day.');
        }
        ...
        try {
            ...
            foreach ($slots_data as $slot) {
                $start = sanitize_input($slot['start'] ?? '');
                $end = sanitize_input($slot['end'] ?? '');
        ...
        if (!in_array($source_day, $allowed_days, true)) {
            send_json_response('error', 'Invalid source day.');
        }
        ...
        try {
            ...
            foreach ($target_days as $day) {
                if (!in_array($day, $allowed_days, true)) {
                    throw new Exception('Invalid target day.');
                }
                // Clear target
                $clear = $pdo->prepare("DELETE FROM time_slots WHERE turf_id = ? AND day_of_week = ?");
                $clear->execute([$turf_id, $day]);
